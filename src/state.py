...
def process_event(state: AppState, event: Event) -> Tuple[AppState, List[Action]]:
    ...
    if isinstance(event, EndOfTurnEvent):
        if event.transcript or event.images or event.audio:
            if state.phase == Phase.RESPONDING:
                # Interrupt current generation and start new turn
                new_state = replace(state, phase=Phase.RESPONDING)
                return new_state, [ResetAgentTurnAction(), StartAgentTurnAction(transcript=event.transcript, images=event.images, audio=event.audio)]
            elif state.phase == Phase.LISTENING:
                new_state = replace(state, phase=Phase.RESPONDING)
                return new_state, [StartAgentTurnAction(transcript=event.transcript, images=event.images, audio=event.audio)]
        return state, []
