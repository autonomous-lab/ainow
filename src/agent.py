...
class Agent:
    ...
    def set_agent(self, name: str) -> None:
        """Switch active agent (CLAUDE.md, sessions dir, memory dir all swap)."""
        self._llm.set_agent(name)

    @property
    def agent_name(self) -> str:
        return self._llm.agent_name
    ...
    def save_session(self, session_id: str) -> str:
        return self._llm.save_session(session_id)

    def load_session(self, session_id: str) -> bool:
        return self._llm.load_session(session_id)

    def switch_model(self, base_url: str, api_key: str, model: str) -> None:
        self._llm.switch_model(base_url, api_key, model)
    ...
    async def start_turn(self, transcript: str, images=None, audio=None) -> None:
        ...
        log.info(f"Turn start: tts_muted={muted}, tts_pool={'yes' if self._tts_pool else 'no'}")

        # Begin tracing this turn
        self._turn = self._tracer.begin_turn(transcript or "[audio]")
        ...
        await self._llm.start(transcript, images=images, audio=audio)
    ...
    async def _on_llm_done(self) -> None:
        ...
        await self._pipeline(
            "llm", "done",
            llm_total=llm_total,
            context_used=self._llm.context_used,
            context_max=self._llm.context_max,
        )
