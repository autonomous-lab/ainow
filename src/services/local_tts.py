...
# Strip emojis and other symbols that TTS would read aloud
...
def _load_model():
    ...
    log.info(f"Loading Kokoro model on {device}...")
    _kokoro_model = KModel(repo_id="hexgrad/Kokoro-82M").to(device).eval()
    log.info(f"Kokoro model ready (24kHz, {device})")
...
class LocalTTSService:
    ...
    async def send(self, text: str) -> None:
        ...
        # Strip emojis so they're not read aloud
