...
### 3. `ainow/call_registry.py` (~50 lines)
...
### 4. `ainow/types.py` -- Add `call_sid` to StreamStartEvent
```python
...
### 5. `ainow/services/twilio_client.py`
...
1. `ainow/models.py` (new, no deps)
2. `ainow/auth.py` (new, no deps)
3. `ainow/call_registry.py` (new, depends on models)
4. `ainow/types.py` (add call_sid field)
5. `ainow/services/twilio_client.py` (extract callSid, add recording param)
6. `ainow/services/llm.py` (configurable constructor)
7. `ainow/services/tts.py` (configurable constructor)
8. `ainow/services/tts_pool.py` (pass voice params through get())
9. `ainow/agent.py` (accept config, add send_first_message)
10. `ainow/conversation.py` (registry integration, first_message, max_duration)
11. `ainow/server.py` (new endpoints, registry wiring, twiml recording)
