...
class TTSPool:
    ...
    def __init__(self, pool_size: int = 1, ttl: float = 8.0, output_format: str = "ulaw_8000"):
    ...
    async def get(
        ...
        tts = TTSService(on_audio=on_audio, on_done=on_done, output_format=self._output_format)
        await tts.start()
        self._trigger_fill()
        return tts
        ...
        try:
            while self._running:
                ...
                while self._running and len(self._ready) < self._pool_size:
                    tts = TTSService(on_audio=_noop_audio, on_done=_noop_done, output_format=self._output_format)
                    try:
                        await tts.start()
                        self._ready.append(
